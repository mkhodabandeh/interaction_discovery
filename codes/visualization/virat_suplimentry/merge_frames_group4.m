...
for g=4%:length(labels)
    ...
    for i=[1 4 3 2 5]
        ...
        for f = startp:endp
            ...
            if i == 1
                bb_v_2 = objm.nearest_vehicle_bb{ls(2)}(f,:)*zoom;
                img = insertShape(img, 'Rectangle', [bb_v_2; bb_v_2+inner], 'Color', 'yellow', 'Opacity', 0);
                
                
                img = insertText(img, bb_v(1:2)+[0 0], 1, 'FontSize', 32, 'BoxOpacity', 0, 'TextColor', 'yellow');
                img = insertText(img, bb_v(1:2)+[20 0], 3, 'FontSize', 32, 'BoxOpacity', 0, 'TextColor', 'yellow');
                img = insertText(img, bb_v(1:2)+[40 0], 5, 'FontSize', 32, 'BoxOpacity', 0, 'TextColor', 'yellow');
            else
            ...
            if first == 1
                ...
                if (i==4) && sum(bb_v_crop==0) == 0 && (bb_v_crop(1) + bb_v_crop(3))<= img_size(2) && (bb_v_crop(2)+bb_v_crop(4)) <= img_size(1)
