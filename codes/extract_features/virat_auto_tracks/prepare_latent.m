...
for scene_counter = 1:size(scenes,1)
    ...
    data_path = ['/cs/vml2/mkhodaba/codes/ECCV14extension/codes/data/VIRAT/' scene '_all.mat'];
    load(data_path);
    ...
    for v = 1:length(videos)
        ...
        for t = 1:length(track_ids)
            ...
            objm.larger_tracklet = cat(1, objm.larger_tracklet, mat2cell([track_inds(start_inds)'; track_inds(end_inds)']', ones(length(start_inds),1)));
