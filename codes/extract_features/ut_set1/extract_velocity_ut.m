...
%vid_name = 'ftr_set2';
        ...
        %{
        %load(['/cs/vml3/avahdat/data/UT/' vid_name '.mat']);
        z = 0.9;
        table = tr_data(:, 3043:3054);
        table_inds = abs(table(:, 9) - 0.8) < 1e-3 & table(:,12) == 5;
        table_zoom8 = table(table_inds, [8 1 3 4 5 6 7 10 2]);
        hoghof_zoom8 = tr_data(table_inds,1:3042);
        table_inds = abs(table(:, 9) - 1) < 1e-3 & table(:,12) == 5;
        table_zoom1 = table(table_inds, [8 1 3 4 5 6 7 10 2]);
        hoghof_zoom1 = tr_data(table_inds,1:3042);
        %table(:,[4:7]) = table(:,[4:7]); %TODO
        
        table = zeros(size(table_zoom1));
        hoghof = zeros(size(hoghof_zoom1));
        for i = 1:size(table_zoom1, 1)
            if table_zoom1(i, 9) <= 5
                table(i,:) = table_zoom8(i,:);
                hoghof(i,:) = hoghof_zoom8(i,:);
            else
                table(i,:) = table_zoom1(i,:);
                hoghof(i,:) = hoghof_zoom1(i,:);
            end
        end
        
        %mkdir('/cs/vml3/mkhodaba/data/UT/');
        save(['/cs/vml3/mkhodaba/data/UT/' vid_name '_small.mat'], 'table');
        save(['/cs/vml3/mkhodaba/data/UT/' vid_name '_small_hoghof.mat'], 'hoghof');
        %}
        load(['/cs/vml3/mkhodaba/data/UT/' vid_name '_small.mat']);
        load(['/cs/vml3/mkhodaba/data/UT/' vid_name '_small_hoghof.mat']);
        ...
        for j = 1:length(ids)
            ...
            %frame nu(used_frames(tracklet_inds),3)mbers of a certain trajectory
        ...
        mkdir([data_address filesep 'UT']);
        
        for i = 1:length(obj.sequence_no)
            if obj.sequence_no(i) > 5
                obj.ten_sec_feature(i) = obj.ten_sec_feature(i)/0.8;
                obj.small_tracklet_features{i} = obj.small_tracklet_features{i}./0.8;
            end
        end
