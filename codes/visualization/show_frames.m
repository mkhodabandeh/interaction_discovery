...
for g=1:length(labels)
    ...
    for i=1:length(tracklet_frames)
        ...
        inner = [1 1 -2 -2];
         mkdir(['/cs/vml3/mkhodaba/codes/ECCV14/results/' dataset_name '/gifs_' scene '/group' num2str(g) '/' num2str(i) '/']);  
        for f = 1:length(tracklet_frames{i})
            ...
            if do_it == 1
                ...
                img = insertShape(img, 'Rectangle', [bb; bb+inner; bb+2*inner], 'Color', 'green', 'Opacity', 0);
                
                if details == 1
                    bb_v = objm.nearest_vehicle_bb{ls(i)}(f,:)*zoom;
                    bb_p = objm.nearest_person_bb{ls(i)}(f,:)*zoom;
                     img = insertShape(img, 'Rectangle', [bb_p; bb_p+inner; bb_p+2*inner], 'Color', 'red', 'Opacity', 0);
                     img = insertShape(img, 'Rectangle', [bb_v; bb_v+inner; bb_v+2*inner], 'Color', 'yellow', 'Opacity', 0);
                    %{
                ...
                imwrite(img, ['/cs/vml3/mkhodaba/codes/ECCV14/results/' dataset_name '/gifs_' scene '/group' num2str(g) '/' num2str(i) '/' num2str(f) '.png']);
                %{
                if f == 1
