...
for g=1:length(labels)
    ...
    mkdir(['/cs/vml3/mkhodaba/codes/ECCV14/results/' dataset_name '/gifs_' scene '/merged/']);
    ...
    for i=1:length(tracklet_frames)
        ...
        for f = 1:5:length(tracklet_frames{i})
            ...
            if i == 1
                ...
                if sum(bb_v_crop==0) == 0 && (bb_v_crop(1) + bb_v_crop(3))<= img_size(2) && (bb_v_crop(2)+bb_v_crop(4)) <= img_size(1)
    ...
    %v_sum
    o_sum
    imwrite(im, map, ['/cs/vml3/mkhodaba/codes/ECCV14/results/' dataset_name '/gifs_' scene '/merged/group' num2str(g) '.gif'], 'DelayTime', 0, 'LoopCount', inf);
end
%}